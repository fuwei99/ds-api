...
// powHeader 的字段顺序即线上 JSON 的 key 顺序，必须与真实网页客户端
// JSON.stringify 的插入序一致；不要改回 map（Go 会按字母序输出 key，
// 与真实客户端不一致，可被上游定点识别）。
...
func BuildPowHeader(c *Challenge, answer int64) (string, error) {
	b, err := json.Marshal(powHeader{
		Algorithm: c.Algorithm, Challenge: c.Challenge, Salt: c.Salt,
		Answer: answer, Signature: c.Signature, TargetPath: c.TargetPath,
	})
	if err != nil {
		return "", err
	}
