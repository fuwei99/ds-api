...
func TestBuildPowHeaderKeyOrder(t *testing.T) {
	header, err := BuildPowHeader(&Challenge{
	...
	want := `{"algorithm":"DeepSeekHashV1","challenge":"abc","salt":"salt","answer":7,"signature":"sig","target_path":"/api/v0/chat/completion"}`
	if string(raw) != want {
