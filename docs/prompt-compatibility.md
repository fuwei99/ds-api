...
- **派生**：`M = HMAC-SHA256(tool_marker_secret, callerID)` 取前 6 个字符，字母表为去掉易混字符（`0/O`、`1/I`）的 `[A-Z2-9]`，并保证两条不变量：**不含 `EPSE` 子串**、**不自重叠**（不存在「自身后缀同时也是自身前缀」的非空 border，例如 `N3KZ5N` 首尾同为 `N`、`ABABAB` 的 `ABAB`）。任一条不满足就换盐重派生（`markerDeriveMaxAttempts`），因此同一个 Key 的 M 仍然稳定。`callerID` 是 API Key 的 `sha256`（`auth.RequestAuth.CallerID`），因此同一个 Key 永远得到同一个 M（多轮 prompt、历史重渲染保持一致），不同 Key 得到不同 M。
...
- 不可调度账号不占名额：被封禁言（`muted_until` 未到期）、已被上游停用（`banned=true`）或已被判定鉴权异常（`auth_failed=true`）的账号一律 `Disabled=true` 且不占用每批 N 个名额，`ReconcileElasticPool` 会继续向后选取可调度账号补位。因此已被停用的账号不会因为优先级高而挤占名额，也无需手动调低其优先级。
- 补位：当账号被上游禁言（补全 `biz_code=5`、文件上传 `biz_code=14`、`biz_msg` 含 `muted`，或 `is_muted=1`）、停用（`USER_IS_BANNED`）或被判定鉴权异常时，客户端在持久化 `muted_until` / `banned` / `auth_failed` 状态的同一事务内执行 `ReconcileElasticPool`：被淘汰账号让出名额，并按上述优先级顺序启用一个休眠账号补上，全程无需人工干预。
...
- 开跑阶段换号重试（Go 主链路）：`CallCompletion` 以及文件上传（`UploadFile` / `ApplyCurrentInputFile` 上传 `HISTORY.txt` / inline 文件上传）在命中禁言时均会持久化 `muted_until` 并上报（`SourceCompletion` / `SourceUpload`），`StartCompletion` 与 `UploadFile` 不会把错误直接透传，而是自动调用 `SwitchAccount` 切到下一个可调度账号并重放本次请求（含 `input_file` 重新上传、分段提示词重新发送）；只要还有可调度账号就继续切换，全部耗尽才向客户端返回 `Account is muted by upstream.`。直传 token 模式（`UseConfigToken=false`）不参与切换。
...
账号级鉴权失败（401）的自动恢复与异常账号淘汰：

- 触发场景：上游以 401 拒绝托管账号的请求（`CreateSession` / `GetPow` / `UploadFile` / `CallCompletion` 等任一环节），或 ds2api 自身归一化出的 `Account token is invalid. Please re-login the account in admin.`。这类错误统一带 `account_unauthorized` 错误码（区别于笼统的 `error`），属于"换号就能救回来"的账号级失败。
- 先刷新 Token、再换号，不中断请求：命中后先对当前账号强制重新登录一次（同一请求内每账号一次，见 `Resolver.RefreshTokenOnce`），刷新成功就用同一账号继续；刷新不成功则 `SwitchAccount` 切到下一个可调度账号重放本次请求。该逻辑由 `completionruntime.retryManagedAuthFailure` 统一实现，`StartCompletion`（开跑阶段，含把上游 401 响应升级为 `account_unauthorized`）、`PrepareCompletionPayload`（Vercel 流式镜像开跑）以及流式/非流式重试循环都走同一条路径。Vercel Node 流式镜像在初始 completion 或重试请求命中 401 时通过 `__stream_switch` 携带 `auth_failed` 交回 Go 侧处理；Go 侧同样先对当前账号执行一次 `RefreshTokenOnce`，刷新成功则保留原账号继续。只有"刷新成功但新 Token 仍被上游拒绝"才额外计入一次账号级鉴权失败；刷新本身失败的原因（登录被上游拒绝 / 网络抖动）不在这一层计数——登录被拒时 `RefreshTokenErr` 内部已经按错误类型计过一次（见 `AuthRefreshAlreadySucceeded` 的判定），网络抖动则不应计数。
- 第二步才判为异常账号：`Resolver.MarkAuthFailure` 记录每个账号在"相邻请求"上的连续鉴权失败次数。首次失败只计数（请求已换号成功，用户无感）；同一账号在下一次真正使用它时再次失败（下一次请求又轮到他、且重新登录仍拿不到可用 Token）即达到阈值，账号被持久化为 `auth_failed=true` + `Disabled=true` + `disabled_reason`，在同一个事务内执行 `ReconcileElasticPool` 让休眠账号补位，并从 `Pool` 取号队列移除。计数按"相邻请求"结算：请求结束时（`Resolver.Release` → `settleAuthFailures`）会把本次成功调度并正常使用过、且没有发生鉴权失败的账号计数清零，因此账号早先偶发失败后正常工作很久，不会因一次新失败被误判为异常。
- 计数只在"账号本身不可用"时累加：只有上游明确拒绝登录（`auth.ErrLoginRejected`，如密码错误、账号被注销）或"重新登录成功但新 Token 仍被拒绝"才计数；网络抖动导致的刷新失败不计入，避免把健康账号误判为异常。账号成功登录（`NoteLoginSuccess`）或成功使用一次（请求结束结算）时计数清零；`NoteLoginSuccess` 还会自动解除此前的 `auth_failed` 判定，使账号重新参与调度。
- 恢复方式：成功刷新 Token 自动解除；管理员在账号管理页手动启用（单个或批量）同样清除 `auth_failed`，便于修好凭据后立即恢复。直传 token 模式（`UseConfigToken=false`）不参与计数与切换。
- 与封号的区别：`banned` 只由 `USER_IS_BANNED` 置位，手动启用不解除；`auth_failed` 由"连续鉴权失败"置位，成功登录或手动启用都会解除。

临时上游故障的自动重试（Go 主链路 + Vercel 镜像）：

- 开跑阶段：`StartCompletion` 会把上游返回的临时 5xx（500 / 502 / 503 / 504）响应升级为可重试的 `upstream_unavailable` 错误，并把 `CallCompletion` 的传输层错误（网络中断等，原 `Failed to get completion.`）同样归类为 `upstream_unavailable`，随后按 `SwitchAccount` 切到下一个可调度账号重放；托管账号（`UseConfigToken=true`）的 401 响应同样在开跑阶段被升级为 `account_unauthorized`（否则流式链路会把原始 401 直接写回客户端），并走"先刷新 Token、再换号"的恢复路径；直传 token 模式（`UseConfigToken=false`）不参与切换，调用方取消（`ctx.Err() != nil`）保持原样透传，不触发切号。
- 非流式重试循环：synthetic retry 请求本身命中的临时 5xx 或传输错误也会先尝试切号，而不是直接返回 `Failed to get completion.`；命中 401 时走 `retryManagedAuthFailure` 的刷新 Token / 换号路径。
...
- Vercel Node 流式镜像：初始 completion 命中临时 5xx 时通过 `__stream_switch` 循环切号；命中 401 时同样切号并携带 `auth_failed` 交回 Go 侧（先刷新 Token，只有"刷新成功但新 Token 仍被拒"才计入一次账号级鉴权失败）；流中解析到临时上游错误时同样交回重试 / 切号循环；重试请求命中临时 5xx 时继续切号。

实现：
- [internal/promptcompat/tools_presence.go](../internal/promptcompat/tools_presence.go) `RequestBodyHasTools`
- [internal/config/account.go](../internal/config/account.go) `Account.MatchesPoolType` / `NormalizePoolType` / `IsAuthFailed` / `IsSchedulable`
- [internal/account/pool_acquire.go](../internal/account/pool_acquire.go) `AccountFilter` / `Acquire` / `AcquireWait`
- [internal/account/elastic.go](../internal/account/elastic.go) `ReconcileElasticPool` / `applyElasticState`
- [internal/auth/request.go](../internal/auth/request.go) `WithToolsPresent` / `Determine` / `acquireManagedRequestAuth` / `SwitchAccount` / `Release`
- [internal/auth/auth_failure.go](../internal/auth/auth_failure.go) `ErrLoginRejected` / `RefreshTokenErr` / `RefreshTokenOnce` / `AuthRefreshAlreadySucceeded` / `MarkAuthFailure` / `NoteLoginSuccess` / `settleAuthFailures`
- [internal/completionruntime/nonstream.go](../internal/completionruntime/nonstream.go) `authOutputError` / `sessionOutputError` / `powOutputError` / `isManagedUnauthorized` / `retryManagedAuthFailure` / `canRetryOnAlternateAccount`
- [internal/deepseek/client/client_auth_failure.go](../internal/deepseek/client/client_auth_failure.go) `handleManagedAuthFailure` / `loginRejectedError` / `noteLoginSuccess`
