...
- 除了显式 `INCOMPLETE` / `AUTO_CONTINUE` 信号，ds2api 还会把「流已结束（EOF 或读错误）但**未出现终态**（FINISHED / CONTENT_FILTER）、且期间产生过可见内容、且未收到 `[DONE]` 哨兵」的流判定为**被截断**，自动发起 continue 补全；单次补全轮数上限为 32。
- 若 continue 补全也失败，代理层会把截断显式暴露为 `502 upstream_interrupted` 错误帧，而不是把残缺输出当作正常完成的 `stop`。
- 已经收到 `[DONE]` 之后连接被 RST/FIN 属于正常收尾，不会触发补全。
...
### 9.1 段游标（sticky cursor）语义

在开启思考（reasoning）的流里，每个 token 归「思考」还是「正文」段，不是单看某个 chunk 的路径就能定，而是由一条贯穿整条流的**粘性段游标（sticky cursor）**决定：

- 游标初始值：`thinking_enabled` 为真时从 `thinking` 起，否则从 `text` 起；`parseChunkForContent` 每次返回的 `newType` 会被调用方赋回游标。
- `THINK` / `THINKING` fragment 一旦出现，游标立即切到 `thinking`；`RESPONSE` fragment 或 `response/content` 路径是**唯一切回 `text`** 的途径。
- 游标是粘性的：后续没有显式标记的裸块（`{"v":"..."}` 这类无路径 chunk）按当前游标归类，不会因为换了 chunk 就自动落回默认段。
- **无前缀相对路径同样遵循粘性游标**：`fragments/-1/content`（response BATCH 内嵌，不带 `response/` 前缀）与 `response/fragments/-N/content` 承载的都是「当前流的段类型」——深度思考时是 thinking 内容、正文时是正文内容，而不是正文标记，因此不硬编码为 `text`。
- **一刀切规则（正文开始后不再输出任何思考）**：一旦本次请求已开始输出正文——即游标已经切到 `text`——之后流里再出现的任何 `THINK` / `THINKING` 增量、`response/thinking_content` 增量或 reasoning 类型片段，一律直接丢弃：不再输出 `thinking_content` / `reasoning_content` 增量，也不把它们当作正文拼接进 content。为此，游标一旦进入 `text` 就**不再回退**到 `thinking`（上游在正文中途再次下发 THINK 属于异常重放，不再回退游标，避免后续正文被误判成思考而吞掉）。这一条覆盖 Go 侧 `internal/sse/parser.go` 与 Vercel Node 镜像 `internal/js/chat-stream/sse_parse_impl.js` 两条链路。
- 嵌套 `fragments` BATCH 里 `{"p":"-1/content","o":"APPEND","v":"..."}` 这类**没有 `content` 字段、只有 `v` 载荷**的增量同样按粘性游标归类；`TIP` 等附注型片段仍跳过。

真实形态示例（response BATCH 内嵌无前缀相对路径，游标处于 thinking 态时归思考段）：

```json
{"p":"response","o":"BATCH","v":[
  {"p":"accumulated_token_usage","v":340},
  {"p":"fragments/-1/content","o":"APPEND","v":" total order"},
  {"p":"quasi_status","o":"SET","v":"INCOMPLETE"}
]}
```

真实形态示例（嵌套 fragments BATCH 内无 `content` 字段的 `v` 载荷，正文态下尾部代码必须保留，`TIP` 跳过）：

```json
{"p":"response","o":"BATCH","v":[{"p":"fragments","o":"BATCH","v":[
...
## 2026-09-10 网页客户端 2.4.0 增量观察（实现补充）

网页客户端升级到 2.4.0（`x-client-version` 基线同步上调，`internal/deepseek/protocol/constants_shared.json`），SSE 解析侧观察到三类行为变化，ds2api 已在 Go（`internal/sse/parser.go`）与 Vercel Node 镜像（`internal/js/chat-stream/sse_parse_impl.js`）同步实现：

1. **`TOOL_` 前缀 fragment 归思考段**：搜索/工具类 fragment（`TOOL_SEARCH` / `TOOL_OPEN` 等）的 `content` 是进度旁白（如「正在搜索图片」「已搜索到相关图片」），不是回答正文。ds2api 现在把这类内容统一归入 thinking / reasoning 输出（`isToolFragmentType`），不再落入可见正文；正文的唯一切换途径仍是 `RESPONSE` fragment / `response/content` 路径。
2. **片段级 `status: FINISHED` 不再终止整条流**：2.4.0 会把 `status: FINISHED` patch 嵌在单个片段作用域路径下发送，例如 `{"p":"response/fragments/-1","v":[{"p":"status","v":"FINISHED"}]}`。这表示**该片段**完成，不是整条响应的终态。若按旧的 `isStatusPath` 规则直接判终态，搜索类流会在「正在搜索图片」片段完成后被误判提前截断。判定函数为 `isFragmentScopedPath`：去掉可选的 `response/` 前缀后形如 `fragments/<index>`（不含更深的 `/`）才算片段作用域；集合追加路径 `response/fragments` 与更深路径 `fragments/-1/content` 不算。此外，片段作用域路径下裸的 `content` / `response/content` patch（搜索片段自己的内容更新）也按旁白归入 thinking。
3. **`expire_at` 毫秒/秒归一**：2.4.0 的 PoW challenge `expire_at` 改用毫秒（旧版为秒）。缓存判定前若发现数值超过秒级上限会先除以 1000 再与 `time.Now().Unix()` 比较，否则过期的缓存 challenge（如文件上传场景）会被误判为长期有效并遭上游拒绝（`internal/deepseek/client/pow.go`）。

配套变化：上传接口现在固定携带 `x-model-type: default`（fast / expert / vision 三种上游 model_type 合并为 `default`，图片由上游服务端按 model_kind VISION 分类）；`upload_file` 返回 `PENDING` / `PARSING`，文件在 completion 流执行期间异步到达 `SUCCESS`，等待解析完成只做尽力而为。详见 [prompt-compatibility.md](./prompt-compatibility.md) 第 8 节。
