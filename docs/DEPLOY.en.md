...
Compatibility note: when `DS2API_CONFIG_PATH` is unset and runtime base dir is `/app`, newer versions prefer `/data/config.json`; if that file is missing but legacy `/app/config.json` exists, DS2API automatically falls back to the legacy path to avoid post-upgrade config loss. For the Mihomo proxy bridge, subscriptions and port mappings are persisted separately in `mihomo_subscriptions.json`. The compose template sets `DS2API_MIHOMO_SUBSCRIPTIONS_PATH=/app/data/mihomo_subscriptions.json` by default to ensure proxy bridge configurations persist inside the mounted `./data` volume across container updates and rebuilds.
...
5. After stream ends, Node calls Go release endpoint (`?__stream_release=1`) to free the account; the request also carries the final `finish_reason` / `usage` / `thinking` / `content` / `error`, which Go uses to backfill chat history and record token usage (release is therefore no longer excluded from chat-history capture)
...
- The Node stream path also mirrors Go finalization semantics: empty visible output returns the same shaped error SSE, and empty `content_filter` returns a `content_filter` error; when the thinking chain contains the upstream moderation refusal text "你好，这个问题我暂时无法回答，让我们换个话题再聊聊吧", it returns 400 `input_content_blocked` (输入内容被审核拦截) without switching accounts
