...
兼容说明：若未设置 `DS2API_CONFIG_PATH` 且运行目录是 `/app`，新版本会优先使用 `/data/config.json`；当该文件不存在但检测到历史 `/app/config.json` 时，会自动回退读取旧路径，避免升级后“配置丢失”。对于代理桥（Mihomo）独立持久化的 `mihomo_subscriptions.json`，Compose 模板默认配置了 `DS2API_MIHOMO_SUBSCRIPTIONS_PATH=/app/data/mihomo_subscriptions.json`，使其直接保存在挂载的 `./data` 卷内，避免重新构建或拉取镜像更新容器后订阅与节点分配丢失。
...
5. 流结束后 Node 调用 Go release 接口（`?__stream_release=1`），释放账号；该请求同时携带本次收尾的 `finish_reason` / `usage` / `thinking` / `content` / `error`，Go 侧据此回填聊天历史并记录 Token 用量（release 因此不再被聊天历史采集逻辑排除）
...
- Node 流式路径同时对齐 Go 的终结态语义：空可见输出会返回同形状错误 SSE，空 `content_filter` 会返回 `content_filter` 错误；当推理链中命中上游审核拒绝文案「你好，这个问题我暂时无法回答，让我们换个话题再聊聊吧」时，返回 400 `input_content_blocked`（输入内容被审核拦截）且不切号
