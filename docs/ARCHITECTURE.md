...
- `internal/completionruntime`：Go surface 共享的 completion 执行辅助，负责 DeepSeek session/PoW/call 启动、非流式 collect、empty-output retry，以及托管账号在最终 429 前的一次切号 fresh retry；流式路径复用它启动上游请求，继续用 `internal/stream` 做实时消费，并在最终收尾阶段接入 `assistantturn`。流式消费器把「上游流中途读错误 / idle / no-content 超时」识别为**断流**：已有部分输出时通过 continue 端点对同一条消息做补全重试（`ResumeContinue`），无消息 id 或补全耗尽时显式返回 `502 upstream_interrupted` 错误帧，而不是把截断伪造成正常 `stop`。
- `internal/translatorcliproxy`：Claude/Gemini 与 OpenAI 结构互转的桥接兼容层，不作为主业务协议转换中心。
- `internal/deepseek/{client,protocol,transport}`：上游请求、会话、PoW 适配、协议常量与传输层。主备传输均基于 httpcloak 的 chrome-150-windows 指纹（TLS ClientHello / H2 / 头顺序一致）；兜底传输仅把协议协商从强制 H2 换成 auto（H2 失败退 H1），不退回裸 Go 标准库（其 crypto/tls 指纹是风控特征）。
...
- `internal/toolcall` + `internal/toolstream`：EPSE 外壳兼容与 canonical XML 工具调用解析、防泄漏筛分；EPSE 会在入口归一化回 XML，内部仍按 XML 语义解析。提示词出口还会把 EPSE 替换成按 API Key 派生的调用者专属标识（`internal/toolcall/marker.go` + `internal/config/tool_marker.go`），模型输出在进入本链路前由有状态归一化器折回 EPSE（Vercel Node 镜像对应 `internal/js/chat-stream/marker.js`），详见 `docs/prompt-compatibility.md` §6.1.2。工具调用失败兜底修复（意图识别 `DetectToolCallIntent`、CDATA 占位符 slot、LLM 修复，见 `docs/toolcall-semantics.md` 第 7 节）也在此链路。
...
- `internal/usagestats`：Token 用量账本，按 GMT+8 日期 + 模型 + 调用方聚合累计数值与费用估算；费用在记录时刻按当时单价一次估算（波峰时段按 GMT+8 解释，倍率计入），不追溯重算；单价 0 表示不计费。波峰时段外的日期豁免由设置里的「排除周末及节假日」控制（JSON 键沿用历史名称 `weekend_normal`）：开启后周末与法定节假日全程按空闲价计价，法定节假日数据来自 `github.com/wangzeping722/chinesecalendar`（覆盖 2004-01-01 至 2026-10-10），超出覆盖范围时自动退化为「仅周末空闲」；该库按 `time.Local` 建索引，因此传入前需把 GMT+8 的年月日重新落到 `time.Local`。模型合并后全部公开模型（`deepseek-v4.1-flash` 及其 `-search` / `-nothinking` 变体）共用同一套单价，因此单价表只保留 `deepseek-v4.1-flash` 这一条（变体按「-」分段命中该键），设置页只提供一组输入/输出单价输入框；加载设置时会把旧档位键（`flash` / `vision` / `pro` / `v4-pro`）与一度单列的 `deepseek-v4.1-flash-search` 键收敛进该条目，其余键一律丢弃。用量页支持把其他部署「导出」的 CSV 再「导入」：前端解析 CSV 成条目后调用 `POST /admin/usage/import`，后端 `Store.Import` 按日期 + 模型 + 调用方与既有账本累加（费用沿用导出值，不重新计价），因此可用于合并多台实例的用量。
- `internal/usagestats` 的「模拟缓存命中」（`cache_hit`）：上游不返回缓存命中信息，因此按设置里的命中率模拟——输入 tokens 拆成命中部分（`input_price`，设置页显示为「输入（缓存命中）」）与未命中部分（`input_miss_price`，默认 1 元/百万 tokens），分别计价后再叠加输出费用与波峰倍率；关闭时输入整体按 `input_price` 计价，与改造前完全一致。默认开启、命中率 98%，前端「各模型 Token 用量」图在 tokens 模式下同样按该命中率把「输入 Tokens」拆成「输入(命中缓存)」与「输入(未命中缓存)」两条堆叠序列（口径与后端 `inputCost` 一致）。`cache_hit.enabled` / `cache_hit.rate` / `input_miss_price` 都用指针解码，以便区分「旧设置文件缺少该键」（回落默认值）与「用户显式关闭 / 显式填 0」；命中率会被钳制到 `[0, 100]`。
