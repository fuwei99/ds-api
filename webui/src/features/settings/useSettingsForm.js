...
const DEFAULT_MUTE_REPORT_URL = 'http://127.0.0.1:8100'
...
const DEFAULT_FORM = {
    ...
    runtime: {
        ...
        device_id_mode: 'random',
        mute_report: { enabled: false, url: DEFAULT_MUTE_REPORT_URL },
        account_rate_limit: { enabled: false, max_per_minute: 5 },
    },
    responses: { store_ttl_seconds: 900 },
    embeddings: { provider: '' },
    auto_delete: { mode: 'none' },
    input_file: { enabled: false, min_chars: 0 },
    thinking_injection: { enabled: false, prompt: '', default_prompt: '' },
    bottom_format_injection: { enabled: true, prompt: '', default_prompt: '' },
    expert_prompt_segment: { enabled: false, max_chars: 160000 },
...
function fromServerForm(data) {
    const currentInputFileEnabled = data.input_file?.enabled ?? false
    return {
        ...
        runtime: {
            ...
            device_id_mode: data.runtime?.device_id_mode === 'real' ? 'real' : 'random',
            mute_report: {
                enabled: Boolean(data.runtime?.mute_report?.enabled),
                url: String(data.runtime?.mute_report?.url || DEFAULT_MUTE_REPORT_URL),
            },
            account_rate_limit: {
                enabled: Boolean(data.runtime?.account_rate_limit?.enabled),
                max_per_minute: Number(data.runtime?.account_rate_limit?.max_per_minute || 5),
            },
        },
        responses: {
        ...
        input_file: {
            ...
            min_chars: Number(data.input_file?.min_chars ?? 0),
        },
        thinking_injection: {
        ...
        // 与「思考格式注入」不同，这里直接把内置提示词预填进输入框（而不是
        // 放在 placeholder 里），用户可以在此基础上修改，保存即覆盖内置文本；
        // 清空输入框则回退到内置默认提示词。
        bottom_format_injection: {
            enabled: data.bottom_format_injection?.enabled ?? true,
            prompt: data.bottom_format_injection?.prompt || data.bottom_format_injection?.default_prompt || '',
            default_prompt: data.bottom_format_injection?.default_prompt || '',
        },
...
function toServerPayload(form) {
    const currentInputFileEnabled = Boolean(form.input_file?.enabled)
    return {
        ...
        runtime: {
            ...
            device_id_mode: form.runtime.device_id_mode === 'real' ? 'real' : 'random',
            mute_report: {
                enabled: Boolean(form.runtime?.mute_report?.enabled),
                url: String(form.runtime?.mute_report?.url || '').trim(),
            },
            account_rate_limit: {
                enabled: Boolean(form.runtime?.account_rate_limit?.enabled),
                max_per_minute: Math.max(1, Number(form.runtime?.account_rate_limit?.max_per_minute || 5)),
            },
        },
        ...
        input_file: {
            enabled: currentInputFileEnabled,
            min_chars: Number(form.input_file?.min_chars ?? 0),
        },
        thinking_injection: {
        ...
        bottom_format_injection: {
            enabled: Boolean(form.bottom_format_injection?.enabled ?? true),
            prompt: String(form.bottom_format_injection?.prompt || '').trim(),
        },
        expert_prompt_segment: {
            enabled: false,
