        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label className="flex items-start gap-3 rounded-lg border border-border bg-background/60 p-4">
                    <input
                        type="checkbox"
                        checked={Boolean(form.bottom_format_injection?.enabled ?? true)}
                        onChange={(e) => setForm((prev) => ({
                            ...prev,
                            bottom_format_injection: {
                                ...prev.bottom_format_injection,
                                enabled: e.target.checked,
                            },
                        }))}
                        className="mt-1 h-4 w-4 rounded border-border"
                    />
                    <div className="space-y-1">
                        <span className="text-sm font-medium block">{t('settings.bottomFormatInjectionEnabled')}</span>
                        <span className="text-xs text-muted-foreground block">{t('settings.bottomFormatInjectionDesc')}</span>
                    </div>
                </label>
                <label className="text-sm space-y-2 md:col-span-2">
                    <span className="text-muted-foreground">{t('settings.bottomFormatInjectionPrompt')}</span>
                    <textarea
                        rows={5}
                        value={form.bottom_format_injection?.prompt || ''}
                        onChange={(e) => setForm((prev) => ({
                            ...prev,
                            bottom_format_injection: {
                                ...prev.bottom_format_injection,
                        ...
                        className="w-full bg-background border border-border rounded-lg px-3 py-2 resize-y min-h-32 font-mono text-xs"
                    />
                    <p className="text-xs text-muted-foreground">{t('settings.bottomFormatInjectionPromptHelp')}</p>
                </label>
            </div>
        </div>
    )
