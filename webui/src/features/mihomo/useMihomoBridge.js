...
export function nodesEqual(a, b) {
    ...
    for (let i = 0; i < a.length; i++) {
        const na = a[i]
        const nb = b[i]
        if (na.node_key !== nb.node_key ||
            na.local_port !== nb.local_port ||
            na.health !== nb.health ||
            na.latency_ms !== nb.latency_ms ||
            na.tested_at !== nb.tested_at ||
            na.health_error !== nb.health_error) {
            return false
        }
        const accA = na.accounts || []
        const accB = nb.accounts || []
        if (accA.length !== accB.length) return false
        for (let j = 0; j < accA.length; j++) {
            if (accA[j].identifier !== accB[j].identifier || accA[j].label !== accB[j].label) {
                return false
            }
        }
    }
...
export function statusEqual(a, b) {
    ...
    return a.running === b.running &&
        ...
        a.health?.available === b.health?.available &&
        a.health?.dead === b.health?.dead
}
...
export default function useMihomoBridge({ authFetch, onMessage, onConfigChanged, t }) {
    ...
    const loadStatus = useCallback(async () => {
        try {
            ...
            setStatus(prev => (statusEqual(prev, data) ? prev : data))
        } catch (_err) {
            setStatus(null)
        }
    ...
    const loadNodes = useCallback(async () => {
        try {
            ...
            setNodes(prev => {
                if (nodesEqual(prev, items)) return prev
