...
export default defineConfig(({ mode }) => ({
    ...
    build: {
        ...
        rollupOptions: {
            output: {
                // Keep the charting library in its own chunks so they are cached
                // independently from the admin page code. echarts and zrender are
                // split apart because together they exceed the 500 kB warning
                // threshold; they are still fetched in parallel.
                advancedChunks: {
                    groups: [
                        {
                            name: 'zrender',
