...
- Tool-calling semantics are aligned between Go and Node runtime: models should output the halfwidth-pipe EPSE shell `<|EPSE|tool_calls>` → `<|EPSE|invoke name="...">` → `<|EPSE|parameter name="...">`; DS2API also accepts EPSE wrapper aliases such as `<epse|tool_calls>` and `<|tool_calls>`, common EPSE separator drift such as `<|EPSE tool_calls>`, collapsed EPSE local names such as `<EPSEtool_calls>`, control-separator drift such as `<EPSE␂tool_calls>` / raw STX `\x02`, CJK angle bracket, fullwidth-bang / ideographic-comma separator drift, PascalCase local-name drift, and trailing attribute separator drift such as `<EPS|parameter name="command"|>...〈/EPS|parameter〉`, `<！EPSE！invoke name=“Bash”>`, `<、EPSE、tool_calls>`, `<DSmartToolCalls>`, or `<EPSEtool_calls※>`, arbitrary protocol prefixes such as `<proto💥tool_calls>`; prompts and examples only demonstrate the EPSE shell and never teach prefixless tags, while the parser keeps backward-compatible parsing for legacy canonical XML `<tool_calls>` → `<invoke name="...">` → `<parameter name="...">`. The scanner normalizes fixed local names (`tool_calls` / `invoke` / `parameter`) with non-structural separators before or after them back to XML before parsing, and also tolerates CDATA opener drift such as `<！[CDATA[` / `<、[CDATA[`; only wrapped tool blocks or the narrow missing-opening-wrapper repair path enter the tool path, while bare `<invoke>` does not count as supported syntax. JSON literal parameter bodies are preserved as structured values, explicit empty or whitespace-only parameters are preserved as empty strings, malformed complete wrappers are released as plain text, and loose CDATA is narrowly repaired at final parse/flush when it can preserve a complete outer tool call. When a `parameter`'s CDATA is missing its own `]]>`, final parse additionally closes it at the enclosing element boundary and reparses, so the value can no longer swallow the following parameter; the reparse is kept only when it yields strictly more tool calls / parameters, and a parameter value that legitimately quotes a tool-call example is preserved byte-for-byte.
...
The WebUI is a single-page app whose client-side routes share paths with some Admin API endpoints (e.g. `/admin/settings`, `/admin/test`, `/admin/import`). So that opening or hard-refreshing those pages works, the server answers **browser navigation requests** (`GET`, no `Authorization`, `Accept` containing `text/html`) with the SPA `index.html`. This covers three cases: unregistered path (404), failed auth (401), and method mismatch (405 — e.g. `GET /admin/test` when only `POST` is registered). Real API calls (with `Authorization`, or an `Accept` without `text/html`) are unaffected and still receive `401` / `404` / `405` (the latter with an `Allow` header).
...
| POST | `/admin/keys` | Admin | Add API key (optional `name`/`remark`/`force_disable_tools`) |
| PUT | `/admin/keys/{key}` | Admin | Update API key metadata and switches |
...
| PUT | `/admin/accounts/{identifier}` | Admin | Update account name/remark/pool_type/priority |
...
| GET | `/admin/device-ids` | Admin | List the manual device_id pool |
| POST | `/admin/device-ids` | Admin | Add one device_id to the pool |
| POST | `/admin/device-ids/delete` | Admin | Delete one device_id from the pool |
...
| POST | `/admin/accounts/banned/delete` | Admin | Delete all upstream-banned accounts |
...
| GET | `/admin/usage` | Admin | Read the token usage ledger (aggregated by GMT+8 date + model + caller, including input/output/reasoning tokens and estimated cost) |
| DELETE | `/admin/usage` | Admin | Clear all token usage data |
| POST | `/admin/usage/import` | Admin | Merge externally exported CSV usage entries (sums by date + model + caller, preserving exported cost instead of re-pricing) |
| GET | `/admin/usage/settings` | Admin | Read usage pricing settings (model prices, peak windows, the "exclude weekends & public holidays" switch, and the "simulate cache hit" switch with its hit rate) |
| PUT | `/admin/usage/settings` | Admin | Update usage pricing settings (model prices, peak windows, the "exclude weekends & public holidays" switch, and the "simulate cache hit" switch with its hit rate) |
...
```json
{
  "object": "list",
  "data": [
    {"id": "deepseek-v4.1-flash", "object": "model", "created": 1677610602, "owned_by": "deepseek", "permission": []},
    {"id": "deepseek-v4.1-flash-nothinking", "object": "model", "created": 1677610602, "owned_by": "deepseek", "permission": []},
    {"id": "deepseek-v4.1-flash-search", "object": "model", "created": 1677610602, "owned_by": "deepseek", "permission": []},
    {"id": "deepseek-v4.1-flash-search-nothinking", "object": "model", "created": 1677610602, "owned_by": "deepseek", "permission": []}
  ]
}
```
...
The DeepSeek web client 2.4.0 merged the old fast / expert / vision modes into a single `default` model type; native model IDs are now only the `deepseek-v4.1-flash` series (all vision-capable). The legacy DeepSeek canonical names (`deepseek-v4-flash`, `deepseek-v4-pro`, `deepseek-v4-vision` and their search variants) remain as built-in aliases that resolve to the corresponding v4.1 models. Alias resolution follows chains (up to 8 hops), so user configs such as `gpt-4o -> deepseek-v4-flash -> deepseek-v4.1-flash` still resolve correctly.
...
```json
{
  ...
  "api_keys": [
    ...
    {"key": "k2", "name": "Backup", "remark": "Load test", "force_disable_tools": true}
  ],
  "env_backed": false,
...
If both `api_keys` and `keys` are sent, the structured `api_keys` entries win so `name` / `remark` / `force_disable_tools` metadata is preserved; `keys` remains a legacy fallback.
...
- `runtime` (`account_max_inflight`, `account_max_queue`, `global_max_inflight`, `token_refresh_interval_hours`, `mute_report`, `account_rate_limit`)
- `responses` / `embeddings`
- `auto_delete` (`mode`: `none` / `single` / `all`; legacy `sessions=true` is still treated as `all`)
- `input_file` (`enabled` defaults to `false`, plus `min_chars`, default `0`)
  - The legacy `current_input_file` key was renamed to `input_file`; the old section is ignored on load (no automatic migration)
- `thinking_injection` (`enabled` defaults to `false`, `prompt`, and `default_prompt`)
- `bottom_format_injection` (`enabled` defaults to `true`, plus `prompt` and `default_prompt`)
- `expert_prompt_segment` (`enabled` always returns `false`, plus `max_chars`; feature disabled, fields kept for backward compatibility)
- `auto_route_vision` (`enabled` always returns `false`; legacy field kept for compatibility — no longer affects routing, since every model is vision-capable after the 2.4.0 merge and images always go through `ref_file_ids`)
- `model_aliases`
...
- `runtime.account_rate_limit` (`enabled` boolean, `max_per_minute` max requests per minute, defaults to false, default limit 5)
- `responses.store_ttl_seconds`
- `embeddings.provider`
- `auto_delete.mode`
- `input_file.enabled` / `input_file.min_chars`
- `thinking_injection.enabled` / `thinking_injection.prompt`
- `bottom_format_injection.enabled` / `bottom_format_injection.prompt`
...
```json
{"key": "new-api-key", "name": "Primary", "remark": "Production", "force_disable_tools": false}
```

`force_disable_tools` is the per-key "force disable tool calling" switch and defaults to `false` (off). When enabled, requests using this key have their tool definitions stripped (both the structured `tools` array and tools declared in system text stop counting) and no tool prompt is injected.

**Response**: `{"success": true, "total_keys": 3}`

### `PUT /admin/keys/{key}`

Updates the `name` / `remark` / `force_disable_tools` of the specified API key. The path `key` is read-only and cannot be changed. Send only the fields you want to change.

```json
{"name": "Backup", "remark": "Load test", "force_disable_tools": true}
```
...
Adds an account. At least one of `email` / `mobile` is required, and `password` is required.

**Request body**:

| Field | Required | Description |
| --- | --- | --- |
| `email` | one of two | Email identifier |
| `mobile` | one of two | Mobile identifier (normalized on save; with/without `+86` is treated as the same account) |
| `password` | yes | Login password |
| `name` | no | Account name |
| `remark` | no | Remark |
| `proxy_id` | no | Proxy ID to bind; must reference an existing proxy |
| `pool_type` | no | Pool type: `default` (default, eligible for any request) / `no_tools` (only requests without tool calls) / `tools_only` (only requests with tool calls). Case-insensitive; unknown values fall back to `default` |
| `priority` | no | Elastic-pool selection priority (integer, negative allowed). Higher values are activated first; `0` is the default and a negative value ranks below default accounts. Accounts with equal priority keep the original config order. Defaults to `0` when omitted |
| `device_id` | no | Explicit device ID. In `manual` mode the server assigns and overwrites this from the pool, so it normally does not need to be set by hand |
| `locale` | no | Locale |

```json
{"email": "user@example.com", "password": "pwd", "pool_type": "no_tools"}
```
...
Updates the `name` / `remark` / `pool_type` / `priority` of the specified account. The path `identifier` can be email or mobile and cannot be changed; fields omitted from the body are left unchanged.

```json
{"name": "Primary account", "remark": "Shared with the team", "pool_type": "tools_only", "priority": 10}
```
...
### `POST /admin/accounts/banned/delete`

Delete all accounts suspended by upstream (`banned=true`), cleaning up their device profiles and device_id generation locks.

**Response**: `{"success": true, "total": 2}`
...
### `GET /admin/device-ids`

Returns the manual device_id pool (only used in `manual` mode).
...
`bound_count` is persisted separately so that requests never have to scan every account.

### `POST /admin/device-ids`

Adds one device_id to the pool.

- Request body: `{"id":"..."}`.
- Both forms are accepted: with the `B` prefix (89 chars) and without it (88 chars). They are told apart by length and stored in the canonical prefixed form.
- Adding an id never binds accounts that have not logged in yet, so ids can be added before any account logs in.
- Returns 400 with a specific reason when the length is not 88/89, the value does not end with `==`, or it is not valid 64-byte base64.
- Adding a duplicate id returns 400.

**Response**: `{"success": true, "id": "B...==", "available": 4}`

### `POST /admin/device-ids/delete`

Deletes one device_id from the pool. Accounts still eligible for binding that were bound to it are immediately rebound to the least-bound remaining id; when the pool becomes empty every account is unbound.

- Request body: `{"id":"..."}` (both forms accepted).
- Returns 404 when the id does not exist.

**Response**: `{"success": true, "available": 2}`
...
```json
{
  ...
  "waiting": 0,
  "max_queue_size": 8,
  "device_id_mode": "manual",
  "device_id_remaining": 3
}
```
...
| `device_id_mode` | Effective device ID mode (`manual` / `real`) |
| `device_id_remaining` | Number of device_id values left in the pool |
...
| `model` | No | `deepseek-v4.1-flash` | Target model |
...
curl http://localhost:5001/v1/chat/completions \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "model": "deepseek-v4.1-flash-search",
