...
- Tool Calling 的解析策略在 Go 与 Node Runtime 间保持一致：推荐模型输出半角管道符 EPSE 外壳 `<|EPSE|tool_calls>` → `<|EPSE|invoke name="...">` → `<|EPSE|parameter name="...">`；兼容层也接受 EPSE wrapper 别名 `<epse|tool_calls>`、`<|tool_calls>`、常见 EPSE 分隔符漏写形态（如 `<|EPSE tool_calls>`）、`EPSE` 与工具标签名黏连的常见 typo（如 `<EPSEtool_calls>`）、控制分隔符漂移（如 `<EPSE␂tool_calls>` / 原始 STX `\x02`）、CJK 尖括号、全角感叹号、顿号、PascalCase 本地名、弯引号属性值与属性尾部分隔符漂移（如 `<EPS|parameter name="command"|>...〈/EPS|parameter〉` / `<！EPSE！invoke name=“Bash”>` / `<、EPSE、tool_calls>` / `<DSmartToolCalls>` / `<EPSEtool_calls※>`）、任意协议前缀壳（如 `<proto💥tool_calls>`）；提示词与示例只示范 EPSE 外壳，不写入旧式无前缀标签，解析层对旧式 canonical XML `<tool_calls>` → `<invoke name="...">` → `<parameter name="...">` 仅保留向后兼容解析。实现上采用结构扫描：只要固定本地标签名是 `tool_calls` / `invoke` / `parameter`，标签名前或标签名后的非结构性分隔符会在解析入口归一化；CDATA 开头也会容错 `<！[CDATA[` / `<、[CDATA[` 这类分隔符漂移；只有 `tool_calls` wrapper 或可修复的缺失 opening wrapper 会进入工具路径，裸 `<invoke>` 不计为已支持语法；流式场景继续执行防泄漏筛分。若参数体本身是合法 JSON 字面量（如 `123`、`true`、`null`、数组或对象），会按结构化值输出，不再一律当作字符串；显式空字符串和纯空白参数会结构化保留为空字符串，是否拒绝缺参由工具执行侧决定；完整但 malformed 的 wrapper 会作为普通文本释放，不会吞掉或伪造成工具调用；若 CDATA 偶发漏闭合，则会在最终 parse / flush 恢复阶段做窄修复，尽量保住已完整包裹的外层工具调用；某个 `parameter` 的 CDATA 漏掉自己的 `]]>` 时，最终 parse 还会按元素边界补上 `]]>` 后重解一次，避免该参数值吞掉后一个参数（重解只在 call 数 / 参数数严格更优时采用，参数正文里合法引用的工具调用示例保持逐字节原样）。
- `Admin API` 将配置与运行时策略分开：`/admin/config*` 管静态配置，`/admin/settings*` 管运行时行为。
- 当上游返回 thinking-only 响应（模型输出了推理链但无可见文本）时，Go 主路径与 Vercel Node 流式路径都会先自动重试一次：以多轮对话 follow-up 方式追加 prompt 后缀 `"Previous reply had no visible output. Please regenerate the visible final answer or tool call now."` 并设置 `parent_message_id` 在同一 DeepSeek session 内让模型重新输出；同账号重试最大 1 次。若同账号重试后仍即将返回 `429 upstream_empty_output`，托管账号模式会在返回 429 前自动切换到下一个可用账号，新建 session，用原始 payload 再 fresh retry 一次。若推理链（thinking）中命中上游审核拒绝文案「你好，这个问题我暂时无法回答，让我们换个话题再聊聊吧」，则不再切换账号，直接返回 400 `input_content_blocked`（输入内容被审核拦截）。
...
WebUI 是单页应用，其前端路由与部分 Admin API 路径同名（如 `/admin/settings`、`/admin/test`、`/admin/import`）。为让浏览器直接访问或刷新这些页面可用，服务端对**浏览器导航请求**（`GET`、无 `Authorization`、`Accept` 含 `text/html`）返回 SPA 的 `index.html`，覆盖三种情况：路径未注册（404）、鉴权失败（401）、以及方法不匹配（405，例如 `GET /admin/test` 而 API 只注册了 `POST`）。真正的 API 调用（带 `Authorization`，或 `Accept` 不含 `text/html`）不受影响，仍按原状态码返回 `401` / `404` / `405`（`405` 附带 `Allow` 头）。
...
| POST | `/admin/keys` | Admin | 添加 API key（可附 name/remark/force_disable_tools） |
| PUT | `/admin/keys/{key}` | Admin | 更新 API key 备注信息与开关 |
...
| PUT | `/admin/accounts/{identifier}` | Admin | 更新账号 name/remark/pool_type/priority |
...
| POST | `/admin/accounts/banned/delete` | Admin | 删除全部被上游停用的账号 |
...
| GET | `/admin/usage` | Admin | 读取 Token 用量账本（按 GMT+8 日期 + 模型 + 调用方聚合，含输入/输出/思考 Tokens 与估算费用） |
| DELETE | `/admin/usage` | Admin | 清空全部 Token 用量数据 |
| POST | `/admin/usage/import` | Admin | 合并导入外部「导出」的 CSV 还原出的用量条目（按日期 + 模型 + 调用方累加，费用沿用导出值，不重新计价） |
| GET | `/admin/usage/settings` | Admin | 读取用量计价设置（模型单价、波峰时段、「排除周末及节假日」开关，以及「模拟缓存命中」开关与命中率） |
| PUT | `/admin/usage/settings` | Admin | 更新用量计价设置（模型单价、波峰时段、「排除周末及节假日」开关，以及「模拟缓存命中」开关与命中率） |
...
DeepSeek 网页客户端 2.4.0 把旧 fast / expert / vision 模式合并为单一 `default` 模型类型，原生模型只保留 `deepseek-v4.1-flash` 系列（全部具备视觉能力）。旧的 DeepSeek 规范名（`deepseek-v4-flash`、`deepseek-v4-pro`、`deepseek-v4-vision` 及其 search 变体）作为内置 alias 保留，继续解析到对应的 v4.1 系列；alias 解析支持链式跟随（最多 8 跳），因此用户配置里 `gpt-4o -> deepseek-v4-flash -> deepseek-v4.1-flash` 这类链式映射也能正确解析。
...
```json
{
  ...
  "api_keys": [
    ...
    {"key": "k2", "name": "备用 Key", "remark": "压测", "force_disable_tools": true}
  ],
  "env_backed": false,
...
如果同时发送 `api_keys` 与 `keys`，优先保留 `api_keys` 中的结构化 `name` / `remark` / `force_disable_tools`；`keys` 仅作为旧格式兼容回退。
...
- `runtime`（`account_max_inflight`、`account_max_queue`、`global_max_inflight`、`token_refresh_interval_hours`、`device_id_mode`、`mute_report`、`account_rate_limit`）
- `responses` / `embeddings`
- `auto_delete`（`mode`：`none` / `single` / `all`；旧配置 `sessions=true` 仍按 `all` 处理）
- `input_file`（`enabled` 默认返回 `false`、`min_chars` 默认 `0`）
  - 旧键 `current_input_file` 已更名为 `input_file`：读取旧配置时该段会被直接忽略（不做自动迁移）
- `thinking_injection`（`enabled` 默认返回 `false`、`prompt`、`default_prompt`）
- `bottom_format_injection`（`enabled` 默认返回 `true`、`prompt`、`default_prompt`）
- `expert_prompt_segment`（`enabled` 固定返回 `false`、`max_chars`；该功能已停用，字段保留以兼容旧配置）
- `auto_route_vision`（`enabled` 固定返回 `false`；字段保留以兼容旧配置，2.4.0 合并后不再影响路由——所有模型原生支持视觉，图片统一经 `ref_file_ids` 处理）
- `model_aliases`
...
- `runtime.account_rate_limit`（`enabled` 开关、`max_per_minute` 每分钟最大请求数，默认关、默认参数 5）
 - `runtime.device_id_mode`（`manual` 默认 / `real`；历史配置中的 `random` 会自动迁移为 `manual`）
   - `manual`：使用账号管理界面手工录入的 device_id 号池。**绑定只发生在账号登录时**：登录时分配「已绑定账号数」最少的 id，账号在启用期间一直复用同一个 id；账号被手动关闭、弹性号池休眠、禁言、封禁或判定为鉴权异常时解除绑定，重新启用后重新分配。新增 device_id 不会给尚未登录的账号预分配绑定，因此可以先加 id 再逐个登录。号池为空时，请求与刷新 Token 会在真正发起上游调用前报错 `请先配置device_id`。登录过程中若上游返回 `RISK_DEVICE_DETECTED`，该 id 会被自动移出号池并给原绑定账号换绑后无感重试，只有号池中最后一个 id 也失效时才报错 `所有device_id均已失效`。
   - `real`：由数美 fp SDK 生成真实设备指纹，需配置数美公钥与 `fp-1.min.js` 资产；接入参数缺失或生成失败时保留账号已有 `device_id`（不轮换指纹），完全没有可用 id 时报错。**Vercel 部署强制 `manual`**：serverless 文件系统只读且实例随用随弃，设备档案与数美接入配置无法持久化，配置的 `real` 会被忽略
- `runtime.devid_public_key`（可选，数美公钥覆盖；为空时依次读取 `data/devid/public_key`、旧版 `data/devid/config.json`，两处均兼容纯文本公钥与 `{"public_key":"..."}` JSON 两种内容）
...
`replace` 模式会按完整配置结构替换（保留 Vercel 同步元信息）；`merge` 模式会合并 `keys`、`api_keys`、`accounts`、`model_aliases`，并覆盖 `admin`、`runtime`、`responses`、`embeddings` 中的非空字段。`auto_delete`、`input_file` 建议通过 `/admin/settings` 或配置文件管理；`compat` 与 `toolcall` 相关字段会被忽略。

> 注意：`merge` 模式不会更新 `auto_delete`、`input_file`。
...
```json
{"key": "new-api-key", "name": "主 Key", "remark": "生产流量", "force_disable_tools": false}
```

`force_disable_tools` 为「强制禁用工具调用」开关，默认 `false`（关闭）。开启后，使用该 Key 的请求会被强制清除工具定义（结构化 `tools` 与 system 文本工具关键词判定都失效），且不注入任何工具提示词。

**响应**：`{"success": true, "total_keys": 3}`

### `PUT /admin/keys/{key}`

更新指定 API key 的 `name` / `remark` / `force_disable_tools`，路径参数中的 `key` 为只读标识，不可修改。只传需要变更的字段。

```json
{"name": "备用 Key", "remark": "压测", "force_disable_tools": true}
```
...
| 字段 | 含义 | 恢复方式 |
| --- | --- | --- |
| `enabled` | 是否可被调度。弹性号池开启时，未进入本批启用集合的账号也是 `false`（休眠） | 弹性号池重算、手动启用 |
| `banned` | 被上游停用（`USER_IS_BANNED`）。不占弹性号池名额 | 仅成功刷新 Token 后自动解除，手动启用不解除 |
| `muted` / `muted_until` | 被上游临时禁言，到点自动恢复。不占弹性号池名额 | 到期自动恢复 |
| `auth_failed` | 鉴权异常：上游持续以 Token 失效拒绝请求，且强制重新登录也拿不到可用 Token。与 `banned` 一样不占弹性号池名额，被判定后立即由休眠账号补位 | 成功刷新 Token 自动解除，或管理员手动启用 |
| `disabled_reason` | 禁用原因说明（如 `账号鉴权持续失败：...`） | — |

账号级异常（禁言 / 停用 / 鉴权异常）都在写入配置的同一个事务里触发弹性号池重算，因此补位账号无需人工干预。
请求过程中若托管账号被上游以 401 拒绝，服务端会先刷新该账号 Token，仍失败则自动换到下一个可调度账号，不会把 401 直接透传给调用方；同一账号在**相邻请求**上连续两次鉴权失败后会被判为 `auth_failed` 并移出号池——两次失败之间只要该账号被成功使用过一次，计数就会在请求结束时清零，因此早先的偶发失败不会累积。

### `POST /admin/accounts`

新增账号。`email` 与 `mobile` 至少填一个，`password` 必填。
...
| --- | --- | --- |
| `email` | 二选一 | 邮箱标识 |
| `mobile` | 二选一 | 手机号标识（存储时归一化，带/不带 `+86` 视为同一账号） |
| `password` | 是 | 登录密码 |
| `name` | 否 | 账号名称 |
| `remark` | 否 | 备注 |
| `proxy_id` | 否 | 绑定代理 ID，必须是已存在的代理 |
| `pool_type` | 否 | 号池类型：`default`（默认，可被任意请求调度）/ `no_tools`（仅无工具调用请求）/ `tools_only`（仅含工具调用请求）。大小写不敏感，未知值按 `default` 处理 |
| `priority` | 否 | 弹性号池选取优先级（整数，允许负数）。数值越大越优先被启用；`0` 为默认，负数表示低于默认账号。同优先级的账号按配置原始顺序选取。省略时为 `0` |
| `device_id` | 否 | 指定设备 ID。`manual` 模式下由服务端从号池分配并覆盖该字段，通常无需手工填写 |
| `locale` | 否 | 语言区域 |
...
更新指定账号的 `name` / `remark` / `pool_type` / `priority`。路径参数中的 `identifier` 可以是 email 或 mobile，且不可修改；请求体中未出现的字段保持不变。

```json
{"name": "主账号", "remark": "团队共享", "pool_type": "tools_only", "priority": 10}
```
...
删除所有被上游停用（`banned=true`）的账号，并清理其设备档案与 device_id 生成锁。

**响应**：`{"success": true, "total": 2}`
...
读取手工 device_id 号池（仅 `manual` 模式使用）。
...
`bound_count` 是单独持久化的已绑定账号数，避免在超多账号场景下每次请求都遍历全量账号。
...
- 请求体：`{"id":"..."}`；
- 接受带 `B` 前缀（89 字符）与不带前缀（88 字符）两种写法，以长度区分，统一存储为带前缀的规范形式；
- 长度不是 88/89、结尾不是 `==`、或不是合法的 64 字节 base64 时返回 400 并给出具体原因；
- 重复添加同一个 id 返回 400。

**响应**：`{"success": true, "id": "B...==", "available": 4}`

### `POST /admin/device-ids/delete`

从号池删除一个 device_id。原先绑定该 id 且仍可绑定的账号会立即换绑到剩余 id 中绑定数最少者；号池被清空后所有账号解绑。

- 请求体：`{"id":"..."}`（同样接受两种写法）；
- id 不存在时返回 404。

**响应**：`{"success": true, "available": 2}`
...
| `device_id_mode` | 当前生效的设备 ID 模式（`manual` / `real`） |
| `device_id_remaining` | 号池中剩余可用的 device_id 数量 |
...
| `model` | 否 | `deepseek-v4.1-flash` | 目标模型 |
