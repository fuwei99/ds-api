...
- `runtime.account_rate_limit`（`enabled` 开关、`max_per_minute` 每分钟最大请求数，默认关、默认参数 5）
 - `runtime.device_id_mode`（`random` 默认 / `real`；real 需数美公钥与 fp-1.min.js 资产，缺失时自动回退 random 且保留已有 `device_id`。**Vercel 部署强制 `random`**：serverless 文件系统只读且实例随用随弃，设备档案与数美接入配置无法持久化，每次冷启动重新生成都会得到新的 device_id，导致设备指纹频繁变化，因此 real 模式在 Vercel 上不可用，配置的 `real` 会被忽略）
- `runtime.devid_public_key`（可选，数美公钥覆盖；为空时依次读取 `data/devid/public_key`、旧版 `data/devid/config.json`，两处均兼容纯文本公钥与 `{"public_key":"..."}` JSON 两种内容，皆无则 real 回退 random）
