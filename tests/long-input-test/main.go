...
// setLoginHeaders 使用与主程序 Login 一致的保守登录头。
// 登录接口对 Chrome 浏览器头敏感，携带 BaseHeaders 会触发异常响应（如 HTTP 202）。
func setLoginHeaders(req *http.Request) {
	for k, v := range dsprotocol.LoginHeaders("") {
		req.Header.Set(k, v)
	}
}
