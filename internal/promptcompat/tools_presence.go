...
// rewritten to strip `tools`, with one exception: an API key that enables the
// "force disable tool calling" switch marks the request via
// DisableToolsForRequest, which removes the tool definition fields and forces
// the result below to false.
//
// A request marked by DisableToolsForRequest (per-API-key "force disable tool
// calling") always reports false, so neither the structured `tools` array nor
// the system-text scan below can classify it as a tools request.
//
