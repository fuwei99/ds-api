...
// firstToolMarker returns the optional caller-specific tool-call marker. It is
// variadic so existing callers (and tests) that predate the marker keep working.
func firstToolMarker(markers []string) string {
	if len(markers) == 0 {
		return ""
	}
...
func NormalizeOpenAIChatRequest(store ConfigReader, req map[string]any, traceID string, toolMarker ...string) (StandardRequest, error) {
	...
	toolsDisabled := ToolsForceDisabledInRequest(req)
	// OpenAI Chat historically ignores `tool_choice` and always uses the default
	// policy; a force-disabled request pins it to `none` instead.
	toolPolicy := DefaultToolChoicePolicy()
	if toolsDisabled {
		toolPolicy = ToolChoicePolicy{Mode: ToolChoiceNone}
	}
	reminder := ResolveToolReminder(store)
	finalPrompt, toolNames := BuildOpenAIPrompt(messagesRaw, req["tools"], traceID, toolPolicy, thinkingEnabled, reminder, marker)
	finalPrompt = toolcall.ApplyToolMarker(finalPrompt, marker)
...
func NormalizeOpenAIResponsesRequest(store ConfigReader, req map[string]any, traceID string, toolMarker ...string) (StandardRequest, error) {
	marker := firstToolMarker(toolMarker)
	model, _ := req["model"].(string)
	...
	toolsDisabled := ToolsForceDisabledInRequest(req)
	reminder := ResolveToolReminder(store)
	var toolPolicy ToolChoicePolicy
	if toolsDisabled {
		toolPolicy = ToolChoicePolicy{Mode: ToolChoiceNone}
	} else {
		parsed, err := parseToolChoicePolicy(req["tool_choice"], req["tools"])
		if err != nil {
