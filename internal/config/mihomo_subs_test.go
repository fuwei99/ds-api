...
func TestMihomoSubscriptionsFallbackMigration(t *testing.T) {
	dir := t.TempDir()
	...
	// 在默认同级目录写旧订阅文件
	legacySubsPath := filepath.Join(dir, "mihomo_subscriptions.json")
	legacyData := MihomoSubscriptionsFile{
		Version: MihomoSubscriptionsFileVersion,
		Subscriptions: []MihomoSubscription{
			{ID: "legacy-sub", URL: "https://example.com/legacy"},
		},
		PortMap: map[string]int{"legacy-sub::node": 10801},
	}
	raw, err := json.Marshal(legacyData)
	if err != nil {
		t.Fatalf("marshal error: %v", err)
	}
	if err := os.WriteFile(legacySubsPath, raw, 0o600); err != nil {
		t.Fatalf("write legacy file error: %v", err)
	}

	// 指定新的目标路径（如 data/mihomo_subscriptions.json）
	targetDir := filepath.Join(dir, "data")
	if err := os.MkdirAll(targetDir, 0o755); err != nil {
		t.Fatalf("mkdir error: %v", err)
	}
	targetSubsPath := filepath.Join(targetDir, "mihomo_subscriptions.json")
	t.Setenv("DS2API_MIHOMO_SUBSCRIPTIONS_PATH", targetSubsPath)

	// 加载配置时应从回退路径加载，并自动回写迁移到目标路径
	store := LoadStore()
	snap := store.Snapshot()
	if len(snap.Mihomo.Subscriptions) != 1 || snap.Mihomo.Subscriptions[0].ID != "legacy-sub" {
		t.Fatalf("expected fallback subscription loaded, got %+v", snap.Mihomo.Subscriptions)
	}
	if snap.Mihomo.PortMap["legacy-sub::node"] != 10801 {
		t.Fatalf("expected fallback port map loaded, got %+v", snap.Mihomo.PortMap)
	}

	if _, err := os.Stat(targetSubsPath); err != nil {
		t.Fatalf("target subscriptions file should be migrated: %v", err)
	}
}
