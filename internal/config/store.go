...
type Store struct {
	...
	// keyForceDisableToolsMap 是 O(1) 的 API key「强制禁用工具调用」索引，
	// 只收录开启了 force_disable_tools 的 key，未命中即视为关闭（默认关）。
	keyForceDisableToolsMap map[string]bool
...
func loadConfig() (Config, bool, error) {
	// CONFIG_JSON bootstrap: 本地无 config.json 时，从环境变量读取原样
	// config.json 内容并落盘生成，之后走正常文件加载。本地已有文件则
	// 一律以文件为准（免得不小心覆盖掉运行时管理员面板的改动）。
	if rawCfg := strings.TrimSpace(os.Getenv("CONFIG_JSON")); rawCfg != "" {
		path := ConfigPath()
		if _, statErr := os.Stat(path); errors.Is(statErr, os.ErrNotExist) {
			if json.Valid([]byte(rawCfg)) {
				if writeErr := writeConfigBytes(path, []byte(rawCfg)); writeErr != nil {
					Logger.Warn("[config] CONFIG_JSON bootstrap failed", "path", path, "error", writeErr)
				} else {
					Logger.Info("[config] bootstrapped config.json from CONFIG_JSON", "path", path)
				}
			} else {
				Logger.Warn("[config] CONFIG_JSON ignored: not valid JSON")
			}
		}
	}
	...
	if err != nil {
		...
		if errors.Is(err, os.ErrNotExist) && bootstrapConfigFromExample(path) {
			if exampleCfg, exampleErr := loadConfigFromFile(path); exampleErr == nil {
				Logger.Info("[config] bootstrapped config.json from config.example.json", "path", path)
				return exampleCfg, false, nil
			}
			Logger.Warn("[config] loaded bootstrapped config.json failed", "path", path)
		}
...
// bootstrapConfigFromExample 在配置文件缺失时，尝试把同目录下的
// config.example.json 复制为 config.json，让全新部署无需手动创建即可启动。
// 返回 true 表示已成功复制；示例文件不存在或复制失败时返回 false（失败会记录告警）。
func bootstrapConfigFromExample(path string) bool {
	...
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			Logger.Warn("[config] read config.example.json failed", "path", examplePath, "error", err)
		}
		return false
	}
	if err := os.WriteFile(path, content, 0o644); err != nil {
		Logger.Warn("[config] copy config.example.json -> config.json failed", "path", path, "error", err)
		return false
	}
	Logger.Info("[config] bootstrapped config.json from config.example.json", "path", path)
	return true
...
// APIKeyForceDisableTools reports whether the managed API key k has the
// "force disable tool calling" switch turned on. Missing or unknown keys return
// false, so the default is off.
func (s *Store) APIKeyForceDisableTools(k string) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.keyForceDisableToolsMap[k]
}
...
func (s *Store) ExportJSONAndBase64() (string, string, error) {
	...
	// tool_marker_secret is server-internal and must never leave the instance in
	// an export payload; a re-import keeps the running secret instead (see the
	// admin config import replace path).
	exportCfg.ToolMarkerSecret = ""
