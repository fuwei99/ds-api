...
const (
	// DeviceIDTypeRandom 表示 device_id 由本地随机生成（默认）。
	DeviceIDTypeRandom = "random"
	// DeviceIDTypeReal 表示 device_id 由数美 fp SDK 真实生成。
	DeviceIDTypeReal = "real"
)

// NormalizeDeviceIDMode 规范化全局设备 ID 生成模式，空值/未知值视为 random。
func NormalizeDeviceIDMode(mode string) string {
	...
	return DeviceIDTypeRandom
}

// NormalizeDeviceIDType 规范化账号 device_id 的类型标注。
// 空值/未知值保持为空：空类型表示 device_id 来源未标注（历史遗留或用户
// 手动指定），视为已固定的设备，不参与模式切换触发的重新生成。
