...
// device_id，因此强制回退 random，即使配置写了 real 也要被忽略。
func TestRuntimeDeviceIDModeForcedRandomOnVercel(t *testing.T) {
	t.Setenv("DS2API_CONFIG_JSON", `{
		"keys":["k1"],
		"accounts":[{"email":"u@example.com","password":"p"}],
	...
	store := LoadStore()
	if got := store.RuntimeDeviceIDMode(); got != DeviceIDTypeRandom {
		t.Fatalf("expected forced random on Vercel, got %q", got)
	}
}
