...
type RuntimeConfig struct {
	...
	// DeviceIDMode 控制账号设备 ID 的生成方式："random"（默认）或 "real"。
	// 切换模式后，device_id_type 与新模式不一致的账号会在下次登录/token
	// 刷新时重新生成并覆盖 device_id。
	// Vercel 部署下强制 random（serverless 无法持久化设备档案与数美配置，
