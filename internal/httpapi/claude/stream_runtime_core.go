...
type claudeStreamRuntime struct {
	...
	// repeatGuard detects the upstream closing-tag repetition loop on the raw
	// text channel so the stream can be cut and finished normally instead of
	// waiting out the idle watchdog.
...
func (s *claudeStreamRuntime) onParsed(parsed sse.LineResult) streamengine.ParsedDecision {
	...
	if s.repeatGuard.Tripped() {
		// 上游陷入闭合标签复读死循环：结束读取，保留已生成内容走正常收尾。
		config.Logger.Warn("[claude_stream] upstream repetition loop detected; cutting stream", "surface", "claude.messages", "stream", true)
		return streamengine.ParsedDecision{Stop: true, StopReason: streamengine.StopReasonRepeatLoop, ContentSeen: contentSeen}
	}
