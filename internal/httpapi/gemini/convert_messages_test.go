...
func TestGeminiMessagesFromRequestPreservesThoughtOnFunctionCallHistory(t *testing.T) {
	...
	prompt, _ := promptcompat.BuildOpenAIPromptForAdapter(got, nil, "", true, promptcompat.DefaultToolReminderConfig())
	...
	if !strings.Contains(prompt, `<|EPSE|invoke name="search_web">`) {
