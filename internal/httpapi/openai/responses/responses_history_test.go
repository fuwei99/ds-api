...
func TestResponsesRecordsResponseHistory(t *testing.T) {
	...
	_ = store.Update(func(c *config.Config) error {
		// Pin the threshold low so the tiny test prompt still crosses it; the
		// production default is 160000 runes of the whole finalized prompt.
		c.InputFile = config.InputFileConfig{Enabled: &enabled, MinChars: 1}
		return nil
	})
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"deepseek-v4.1-flash","input":"hello responses"}`))
