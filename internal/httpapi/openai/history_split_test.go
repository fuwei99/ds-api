...
func (streamStatusManagedAuthStub) ForceDisableToolsForRequest(_ *http.Request) bool { return false }
...
func TestApplyCurrentInputFileSkipsShortInputWhenThresholdNotReached(t *testing.T) {
	...
	if len([]rune(stdReq.FinalPrompt)) >= 10000 {
		t.Fatalf("test fixture prompt unexpectedly reaches the threshold: %d runes", len([]rune(stdReq.FinalPrompt)))
	}
	...
	if len(ds.uploadCalls) != 0 {
		t.Fatalf("expected no upload when final prompt is below threshold, got %d", len(ds.uploadCalls))
	}
	if out.FinalPrompt != stdReq.FinalPrompt {
		t.Fatalf("expected prompt unchanged when threshold not reached")
	}
}

func TestApplyCurrentInputFileTriggersOnWholeFinalPromptLength(t *testing.T) {
	ds := &inlineUploadDSStub{}
	h := &openAITestSurface{
		Store: mockOpenAIConfig{
			currentInputEnabled: true,
	...
	// Each individual turn is short on its own; the threshold is only crossed
	// once the whole multi-turn prompt is counted.
	messages := []any{
		map[string]any{"role": "system", "content": "system instructions"},
	}
	for i := 0; i < 40; i++ {
		messages = append(messages, map[string]any{"role": "user", "content": "short turn"})
		messages = append(messages, map[string]any{"role": "assistant", "content": "short reply"})
	}
	messages = append(messages, map[string]any{"role": "user", "content": "hi"})
	req := map[string]any{
		"model":    "deepseek-v4.1-flash",
		"messages": messages,
	}
	...
	if len([]rune(stdReq.FinalPrompt)) < 200 {
		t.Fatalf("test fixture prompt unexpectedly short: %d runes", len([]rune(stdReq.FinalPrompt)))
	}
	h.Store = mockOpenAIConfig{
		currentInputEnabled: true,
		// Only the full final prompt exceeds this; no single turn does.
		currentInputMin: len([]rune(stdReq.FinalPrompt)) - 1,
	}
	...
	if !out.CurrentInputFileApplied {
		t.Fatalf("expected split to trigger when the whole final prompt reaches the threshold")
	}
...
func TestApplyCurrentInputFilePreservesFullContextPromptForTokenCounting(t *testing.T) {
	...
	if strings.Contains(out.FinalPrompt, "first user turn") {
		t.Fatalf("expected live prompt to hide earlier turns, got %q", out.FinalPrompt)
	}
	// The trailing user turn stays inline in the live prompt; only the preceding
	// turns move into HISTORY.txt.
	if !strings.Contains(out.FinalPrompt, "[System]:继续会话[User]:latest user turn") {
		t.Fatalf("expected live prompt to keep the trailing user turn inline, got %q", out.FinalPrompt)
	}
}
...
func TestApplyCurrentInputFileUploadsFullContextFile(t *testing.T) {
	...
	if upload.ModelType != "default" {
		t.Fatalf("expected default model type for merged request, got %q", upload.ModelType)
	}
	uploadedText := string(upload.Data)
	// The trailing user turn (and the thinking injection appended to it) stays
	// inline in the live prompt, so HISTORY.txt ends at the tool result.
	for _, want := range []string{"# HISTORY.txt", "=== 1. SYSTEM ===", "=== 2. USER ===", "=== 3. ASSISTANT ===", "=== 4. TOOL ===", "system instructions", "first user turn", "hidden reasoning", "tool result"} {
		if !strings.Contains(uploadedText, want) {
			t.Fatalf("expected full context file to contain %q, got %q", want, uploadedText)
		}
	}
	if strings.Contains(uploadedText, "=== 5. USER ===") || strings.Contains(uploadedText, "latest user turn") {
		t.Fatalf("expected trailing user turn to stay out of HISTORY.txt, got %q", uploadedText)
	}
	if strings.Contains(out.FinalPrompt, "first user turn") || strings.Contains(out.FinalPrompt, "CURRENT_USER_INPUT.txt") || strings.Contains(out.FinalPrompt, "Read that file") {
		t.Fatalf("expected live prompt to only carry the continuation instruction and the latest turn, got %s", out.FinalPrompt)
	}
	if !strings.Contains(out.FinalPrompt, "继续会话") {
		t.Fatalf("expected continuation-oriented prompt in live prompt, got %s", out.FinalPrompt)
	}
	if !strings.Contains(out.FinalPrompt, "[System]:继续会话[User]:latest user turn") {
		t.Fatalf("expected continuation system turn followed by the inline latest user turn, got %s", out.FinalPrompt)
	}
	if !strings.Contains(out.FinalPrompt, promptcompat.ThinkingInjectionMarker) {
		t.Fatalf("expected thinking injection to follow the inline latest user turn, got %s", out.FinalPrompt)
	}
	if strings.Contains(out.FinalPrompt, "[User]:继续会话") {
		t.Fatalf("continuation prompt must not render as a user turn, got %s", out.FinalPrompt)
	}
}
...
func TestApplyCurrentInputFileUploadsToolsContextSeparately(t *testing.T) {
	...
	if !strings.Contains(out.FinalPrompt, "[System]:继续会话") {
		t.Fatalf("expected continuation prompt to render as a system turn, got %q", out.FinalPrompt)
	}
...
func TestApplyCurrentInputFilePreservesReadGuardOnAbnormalResult(t *testing.T) {
	ds := &inlineUploadDSStub{}
	h := &openAITestSurface{
		Store: mockOpenAIConfig{
			currentInputEnabled: true,
	...
	req := map[string]any{
		...
		"messages": []any{
			map[string]any{"role": "system", "content": "system instructions"},
			map[string]any{"role": "user", "content": "read the file please"},
			map[string]any{
				"role": "assistant",
				"tool_calls": []any{
					map[string]any{
			...
			map[string]any{"role": "user", "content": "what should we do next?"},
		},
		"tools": []any{
			map[string]any{
				"type": "function",
	...
	if !strings.Contains(out.FinalPrompt, "Read-tool cache guard") {
		t.Fatalf("expected continuation prompt to retain Read-tool cache guard, got %q", out.FinalPrompt)
	}
}

func TestApplyCurrentInputFileOmitsReadGuardOnNormalResult(t *testing.T) {
	ds := &inlineUploadDSStub{}
	h := &openAITestSurface{
		Store: mockOpenAIConfig{
			currentInputEnabled: true,
	...
	req := map[string]any{
		...
		"messages": []any{
			...
			map[string]any{
				...
				"name":         "read_file",
				"tool_call_id": "call-1",
				"content":      "full file content here: line 1, line 2",
			},
			map[string]any{"role": "user", "content": "what should we do next?"},
		},
		"tools": []any{
	...
	if strings.Contains(out.FinalPrompt, "Read-tool cache guard") {
		t.Fatalf("expected continuation prompt to omit Read-tool cache guard on normal result, got %q", out.FinalPrompt)
	}
}
...
func TestChatCompletionsCurrentInputFileUploadsContextAndKeepsNeutralPrompt(t *testing.T) {
	...
	if !strings.Contains(historyText, "=== 4. TOOL ===") || !strings.Contains(historyText, "tool result") {
		t.Fatalf("expected full context up to the trailing user turn, got %s", historyText)
	}
	if strings.Contains(historyText, "latest user turn") {
		t.Fatalf("expected trailing user turn to stay inline in the live prompt, got %s", historyText)
	}
	...
	if strings.Contains(promptText, "first user turn") {
		t.Fatalf("expected prompt to hide earlier turns, got %s", promptText)
	}
	if !strings.Contains(promptText, "[System]:继续会话[User]:latest user turn") {
		t.Fatalf("expected prompt to keep the trailing user turn inline, got %s", promptText)
	}
	...
	neutralCount := util.CountPromptTokens(promptText, "deepseek-v4.1-flash")
...
func TestCurrentInputFileWorksAcrossAutoDeleteModes(t *testing.T) {
	for _, mode := range []string{"none", "single", "all"} {
		t.Run(mode, func(t *testing.T) {
			...
			promptText, _ := ds.completionReq["prompt"].(string)
			if !strings.Contains(promptText, "[System]:继续会话[User]:latest user turn") || strings.Contains(promptText, "first user turn") {
