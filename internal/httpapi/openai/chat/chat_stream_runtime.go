...
type chatStreamRuntime struct {
	...
	// tool-call repair into the streaming finalize path (M4). They are nil
	// unless the handler enabled repair; the per-chunk sieve never sees them.
...
func (s *chatStreamRuntime) onParsed(parsed sse.LineResult) streamengine.ParsedDecision {
	...
	if accumulated.RepeatLoop {
		// 上游陷入闭合标签复读死循环：立刻结束读取，把已生成内容交给
		// 正常收尾（tool 筛分 / 修复 / finish 帧），仍返回 200。
		config.Logger.Warn("[openai_chat_stream] upstream repetition loop detected; cutting stream", "surface", "chat.completions", "stream", true)
