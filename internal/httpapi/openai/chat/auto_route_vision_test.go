...
func TestChatCompletionsAutoRouteVisionUsesDefaultModelTypeAndPreservesResponseModel(t *testing.T) {
	...
	reqBody := `{"model":"deepseek-v4.1-flash","messages":[{"role":"user","content":[{"type":"text","text":"describe this"},{"type":"image_url","image_url":{"url":"data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg=="}}]}],"stream":false}`
	...
	if got := out["model"]; got != "deepseek-v4.1-flash" {
...
func TestChatCompletionsAutoRouteVisionDisabledKeepsOriginalModel(t *testing.T) {
	...
	reqBody := `{"model":"deepseek-v4.1-flash","messages":[{"role":"user","content":[{"type":"text","text":"describe this"},{"type":"image_url","image_url":{"url":"data:image/png;base64,abc"}}]}],"stream":false}`
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(reqBody))
	req.Header.Set("Authorization", "Bearer direct-token")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
...
func TestChatCompletionsAutoRouteVisionNoImageKeepsOriginalModel(t *testing.T) {
	...
	reqBody := `{"model":"deepseek-v4.1-flash","messages":[{"role":"user","content":"hello"}],"stream":false}`
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(reqBody))
	req.Header.Set("Authorization", "Bearer direct-token")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
...
func TestChatCompletionsAutoRouteVisionPreservesNoThinking(t *testing.T) {
	...
	reqBody := `{"model":"deepseek-v4.1-flash-nothinking","messages":[{"role":"user","content":[{"type":"text","text":"describe this"},{"type":"image_url","image_url":{"url":"data:image/png;base64,abc"}}]}],"stream":false}`
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(reqBody))
	req.Header.Set("Authorization", "Bearer direct-token")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	...
	if payload["model_type"] != "default" {
		t.Fatalf("expected completion model_type=default, got %v", payload["model_type"])
	}
	if payload["thinking_enabled"] != false {
		t.Fatalf("expected thinking_enabled=false for nothinking, got %v", payload["thinking_enabled"])
	}
	out := decodeJSONBody(t, rec.Body.String())
	if got := out["model"]; got != "deepseek-v4.1-flash-nothinking" {
		t.Fatalf("expected response model deepseek-v4.1-flash-nothinking, got %v", got)
	}
}
...
func TestChatCompletionsAutoRouteVisionFollowUpWithoutImageReturnsToOriginalModel(t *testing.T) {
	...
	reqBody := `{"model":"deepseek-v4.1-flash","messages":[{"role":"user","content":[{"type":"text","text":"describe this"},{"type":"image_url","image_url":{"url":"data:image/png;base64,abc"}}]},{"role":"assistant","content":"I see"},{"role":"user","content":"now text only"}],"stream":false}`
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(reqBody))
	req.Header.Set("Authorization", "Bearer direct-token")
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
