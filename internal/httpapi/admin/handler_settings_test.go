...
func TestGetSettingsIncludesCurrentInputFileDefaults(t *testing.T) {
	...
	var body map[string]any
	_ = json.Unmarshal(rec.Body.Bytes(), &body)
	currentInputFile, _ := body["input_file"].(map[string]any)
	if got := boolFrom(currentInputFile["enabled"]); got {
		t.Fatalf("expected input_file.enabled=false, body=%v", body)
	}
	if got := intFrom(currentInputFile["min_chars"]); got != 0 {
		t.Fatalf("expected input_file.min_chars=0, got %d body=%v", got, body)
	}
...
func TestUpdateSettingsCurrentInputFile(t *testing.T) {
	...
	if snap.InputFile.Enabled == nil || !*snap.InputFile.Enabled {
		t.Fatalf("expected input_file.enabled=true, got %#v", snap.InputFile)
	}
	if snap.InputFile.MinChars != 12345 {
		t.Fatalf("expected input_file.min_chars=12345, got %#v", snap.InputFile)
	}
...
func TestUpdateSettingsCurrentInputFilePartialUpdatePreservesEnabled(t *testing.T) {
	h := newAdminTestHandler(t, `{"keys":["k1"],"input_file":{"enabled":false,"min_chars":777}}`)
	payload := map[string]any{
		"input_file": map[string]any{
			"min_chars": 5000,
		},
	...
	snap := h.Store.Snapshot()
	if snap.InputFile.Enabled == nil || *snap.InputFile.Enabled {
		t.Fatalf("expected input_file.enabled to remain false, got %#v", snap.InputFile.Enabled)
	}
	if snap.InputFile.MinChars != 5000 {
		t.Fatalf("expected input_file.min_chars=5000, got %#v", snap.InputFile)
	}
}
...
func TestUpdateSettingsCurrentInputFilePartialUpdatePreservesMinChars(t *testing.T) {
	...
	snap := h.Store.Snapshot()
	if snap.InputFile.Enabled == nil || !*snap.InputFile.Enabled {
		t.Fatalf("expected input_file.enabled=true, got %#v", snap.InputFile.Enabled)
	}
	if snap.InputFile.MinChars != 777 {
		t.Fatalf("expected input_file.min_chars to remain 777, got %#v", snap.InputFile)
	}
}
...
func TestUpdateSettingsIgnoresHistorySplitPayload(t *testing.T) {
	...
	snap := h.Store.Snapshot()
	if snap.InputFile.Enabled == nil || !*snap.InputFile.Enabled {
		t.Fatalf("expected input_file to remain enabled, got %#v", snap.InputFile.Enabled)
	}
}
...
func TestUpdateSettingsAutoRouteVision(t *testing.T) {
	...
	if snap.AutoRouteVision.Enabled == nil || !*snap.AutoRouteVision.Enabled {
		t.Fatalf("expected auto_route_vision.enabled=true to be persisted, got %#v", snap.AutoRouteVision)
	}
	if h.Store.AutoRouteVisionEnabled() {
		t.Fatal("expected auto route vision accessor to stay force-disabled")
	}
}
...
func TestUpdateConfigReplacesModelAliases(t *testing.T) {
	h := newAdminTestHandler(t, `{
		"keys":["k1"],
		"model_aliases":{"claude-sonnet-4-6":"deepseek-v4.1-flash"}
	}`)
	...
	if snap.ModelAliases["gpt-5.5"] != "deepseek-v4.1-flash" {
