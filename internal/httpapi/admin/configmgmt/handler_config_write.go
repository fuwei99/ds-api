...
func (h *Handler) addKey(w http.ResponseWriter, r *http.Request) {
	...
	forceDisableTools, _ := fieldBoolOptional(req, "force_disable_tools")
	if key == "" {
	...
	err := h.Store.Update(func(c *config.Config) error {
		...
		c.APIKeys = append(c.APIKeys, config.APIKey{
			Key:               key,
			Name:              name,
			Remark:            remark,
			ForceDisableTools: forceDisableTools,
		})
		return nil
	})
	if err != nil {
...
func (h *Handler) updateKey(w http.ResponseWriter, r *http.Request) {
	...
	forceDisableTools, forceDisableToolsOK := fieldBoolOptional(req, "force_disable_tools")
	...
	err := h.Store.Update(func(c *config.Config) error {
		...
		if forceDisableToolsOK {
			c.APIKeys[idx].ForceDisableTools = forceDisableTools
		}
		return nil
	})
	if err != nil {
