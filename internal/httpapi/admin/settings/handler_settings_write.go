...
func (h *Handler) updateSettings(w http.ResponseWriter, r *http.Request) {
	...
	adminCfg, runtimeCfg, responsesCfg, embeddingsCfg, autoDeleteCfg, currentInputCfg, thinkingInjCfg, bottomFmtInjCfg, expertPromptSegCfg, autoRouteVisionCfg, aliasMap, err := parseSettingsUpdateRequest(req)
	if err != nil {
	...
	currentInputEnabledSet := hasNestedSettingsKey(req, "input_file", "enabled")
	currentInputMinCharsSet := hasNestedSettingsKey(req, "input_file", "min_chars")
	...
	bottomFormatInjectionEnabledSet := hasNestedSettingsKey(req, "bottom_format_injection", "enabled")
	bottomFormatInjectionPromptSet := hasNestedSettingsKey(req, "bottom_format_injection", "prompt")
	...
	muteReportSet := hasNestedSettingsKey(req, "runtime", "mute_report")
	accountRateLimitSet := hasNestedSettingsKey(req, "runtime", "account_rate_limit")
	deviceIDModeSet := hasNestedSettingsKey(req, "runtime", "device_id_mode")
	...
	if err := h.Store.Update(func(c *config.Config) error {
		...
		if runtimeCfg != nil {
			...
			if runtimeCfg.DeviceIDMode != "" {
				c.Runtime.DeviceIDMode = runtimeCfg.DeviceIDMode
			}
			if muteReportSet {
				if runtimeCfg.MuteReport.Enabled != nil {
					c.Runtime.MuteReport.Enabled = runtimeCfg.MuteReport.Enabled
				}
				c.Runtime.MuteReport.URL = runtimeCfg.MuteReport.URL
			}
			if accountRateLimitSet {
				if runtimeCfg.AccountRateLimit.Enabled != nil {
					c.Runtime.AccountRateLimit.Enabled = runtimeCfg.AccountRateLimit.Enabled
				}
				if runtimeCfg.AccountRateLimit.MaxPerMinute > 0 {
					c.Runtime.AccountRateLimit.MaxPerMinute = runtimeCfg.AccountRateLimit.MaxPerMinute
				}
			}
		}
		...
		if currentInputCfg != nil {
			if currentInputEnabledSet {
				c.InputFile.Enabled = currentInputCfg.Enabled
			}
			if currentInputMinCharsSet {
				c.InputFile.MinChars = currentInputCfg.MinChars
			}
		}
		...
		if bottomFmtInjCfg != nil {
			if bottomFormatInjectionEnabledSet {
				c.BottomFormatInjection.Enabled = bottomFmtInjCfg.Enabled
			}
			if bottomFormatInjectionPromptSet {
				c.BottomFormatInjection.Prompt = bottomFmtInjCfg.Prompt
			}
		}
		...
		if deviceIDModeSet {
			// 模式切换后重算 device_id 绑定：从 real 切回 manual 时，账号持有的
			// 真实 device_id 不在号池中，需要重新分配手工 id。
