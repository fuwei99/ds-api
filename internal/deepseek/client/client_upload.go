...
func (c *Client) UploadFile(ctx context.Context, a *auth.RequestAuth, req UploadFileRequest, maxAttempts int) (*UploadFileResult, error) {
	...
	// The 2.4.0 web client merged fast / expert / vision into a single upstream
	// "default" model_type and always sends x-model-type: default on upload_file
	// (even for images, which are classified server-side as model_kind VISION).
	modelType := "default"
	...
	for attempts < maxAttempts {
		...
		if resp.StatusCode == http.StatusOK && code == 0 && bizCode == 0 {
			...
			// The 2.4.0 web flow parses files asynchronously: upload_file
			// returns PENDING/PARSING and the file reaches SUCCESS while the
			// completion stream runs. Waiting is best-effort so a slow parse
			// never fails the upload; the completion carries the file_id and
			// the upstream finishes parsing it server-side.
			if err := c.waitForUploadedFile(ctx, a, result); err != nil {
				config.Logger.Warn("[upload_file] file not ready before completion; proceeding", "file_id", result.ID, "error", err)
			}
			return result, nil
		}
		...
		if isMutedJSONResponse(parsed) {
			muteUntil := extractMuteUntil(parsed)
			c.persistMutedUntil(a.AccountID, muteUntil, banreport.SourceUpload)
			lastFailureKind = FailureMuted
			lastFailureMessage = failureMessage(msg, bizMsg, "user is muted")
			if a.UseConfigToken && c.Auth != nil && c.Auth.SwitchAccount(ctx, a) {
				refreshed = false
				attempts++
				continue
			}
			return nil, &RequestFailure{Op: "upload file", Kind: FailureMuted, Message: lastFailureMessage}
		}
		...
		if a.UseConfigToken {
			if shouldAttemptRefresh(resp.StatusCode, code, bizCode, msg, bizMsg) && c.handleManagedAuthFailure(ctx, a, &refreshed) {
				attempts++
				continue
			}
