...
func (c *Client) CallCompletion(ctx context.Context, a *auth.RequestAuth, payload map[string]any, powResp string, maxAttempts int) (*http.Response, error) {
	...
	if resp.StatusCode == http.StatusOK {
		...
		if muted {
			_ = resp.Body.Close()
			c.persistMutedUntil(a.AccountID, muteUntil, banreport.SourceCompletion)
...
func (c *Client) streamPostWithFallback(ctx context.Context, doer trans.Doer, url string, headers map[string]string, payload any, allowFallback bool) (*http.Response, error) {
	...
	if err != nil {
		if allowFallback {
			config.Logger.Warn("[deepseek] fingerprint stream request failed, fallback to fingerprinted H1 transport", "url", url, "error", err)
...
// Matches on biz_code==5 (completion), biz_code==14 (upload), biz_msg containing "muted", or is_muted==1 in biz_data.
func isMutedJSONResponse(resp map[string]any) bool {
	...
	if bizCode == 5 || bizCode == 14 || strings.Contains(bizMsg, "muted") {
		return true
	}
...
func extractMuteUntil(resp map[string]any) float64 {
	...
	if muteUntil, ok := data["mute_until"].(float64); ok && muteUntil > 0 {
		return muteUntil
	}
	bizData, _ := data["biz_data"].(map[string]any)
	if bizData != nil {
		if muteUntil, ok := bizData["mute_until"].(float64); ok && muteUntil > 0 {
			return muteUntil
		}
	}
	return 0
