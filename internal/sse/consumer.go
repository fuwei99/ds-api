...
type CollectResult struct {
	...
	// RepeatLoop reports that collection was cut short because the upstream
	// text degenerated into a closing-tag repetition loop (see
	// RepeatLoopGuard). The content collected before the loop is returned
	// as-is.
	RepeatLoop bool
}
...
func CollectStream(resp *http.Response, thinkingEnabled bool, closeBody bool) CollectResult {
	...
	_ = dsprotocol.ScanSSELines(resp, func(line []byte) bool {
		...
		if repeatGuard.Tripped() {
			// Upstream degenerated into a closing-tag repetition loop: stop
			// reading and keep what was collected so the caller's normal
			// finish path (tool sieve / repair / usage) still runs.
			return false
		}
		return true
	})
	return CollectResult{
		...
		RepeatLoop:            repeatGuard.Tripped(),
	}
}
