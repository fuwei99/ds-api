...
func TestDetectToolCallIntentMatrix(t *testing.T) {
	cases := []struct {
		...
		// DSML-format tool output: any DSML-prefixed tag (open or close, any
		// suffix) flags intent so the whole fragment goes to proactive repair.
		{"dsml full shell fullwidth", `<｜｜DSML｜｜calls>x</｜｜DSML｜｜calls>`, true},
		{"dsml open only with attributes", `<｜｜DSML｜｜invoke name="read">`, true},
		{"dsml close only", `</｜｜DSML｜｜invoke>`, true},
		{"dsml ascii pipes with spaces", `<| | DSML | | calls>`, true},
		// A single DSML hit amid prose still flags intent.
		{"dsml hit amid prose", `let me read the file <｜｜DSML｜｜ invoke name="read"> now`, true},
		// DSML literal inside a fully closed fence -> excluded.
		{"dsml markdown fenced", "```\n<｜｜DSML｜｜calls>x</｜｜DSML｜｜calls>\n```", false},
		// DSML literal inside CDATA -> excluded.
		{"dsml cdata literal", `<![CDATA[<｜｜DSML｜｜calls>x</｜｜DSML｜｜calls>]]>`, false},
		// Prose mentioning DSML without a tag shape -> not intent.
		{"prose mentions dsml", `the DSML format is not supported here`, false},
		// "dsml" as a substring of a longer word must not match.
		{"dsml substring word", `<dsmlfoo>x</dsmlfoo>`, false},
	}
...
func TestSawToolCallIntentMatrix(t *testing.T) {
	cases := []struct {
		...
		// DSML-format tool output: nothing parses (wrapper local name is
		// outside the markup name table), and the DSML tags stay in the
		// residual -> intent true, whole fragment routed to repair.
		{
			name:       "dsml shell unparseable keeps intent",
			text:       `<｜｜DSML｜｜calls><｜｜DSML｜｜invoke name="read"><｜｜DSML｜｜parameter name="path"><![CDATA[/tmp/a.txt]]></｜｜DSML｜｜parameter></｜｜DSML｜｜invoke></｜｜DSML｜｜calls>`,
			wantIntent: true,
			wantCalls:  0,
		},
	}
