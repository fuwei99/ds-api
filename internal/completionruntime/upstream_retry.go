...
func promoteTransientStartResponse(start *StartResult, a *auth.RequestAuth, outErr *assistantturn.OutputError) *assistantturn.OutputError {
	if outErr != nil || start == nil || start.Response == nil {
		return outErr
	}
	if transientErr := transientUpstreamResponseError(start.Response); transientErr != nil {
		start.Response = nil
		return transientErr
	}
	// 托管账号被上游以 401 拒绝同样属于"换号就能救回来"的失败：不升级的话
	// 流式链路会把原始 401 直接写回客户端，弹性号池完全失去意义。
	if authErr := managedUnauthorizedResponseError(start.Response, a); authErr != nil {
		start.Response = nil
		return authErr
	}
	return nil
...
// managedUnauthorizedResponseError converts a 401 start response for a managed
// account into the shared account_unauthorized error, consuming and closing the
// body. It returns nil when the response is not an auth rejection.
func managedUnauthorizedResponseError(resp *http.Response, a *auth.RequestAuth) *assistantturn.OutputError {
	if resp == nil || a == nil || !a.UseConfigToken || resp.StatusCode != http.StatusUnauthorized {
		return nil
	}
	if resp.Body != nil {
		body, readErr := io.ReadAll(io.LimitReader(resp.Body, 64*1024))
		if readErr != nil {
			config.Logger.Warn("[completion_runtime] unauthorized response body read failed", "status", resp.StatusCode, "error", readErr)
		}
		if closeErr := resp.Body.Close(); closeErr != nil {
			config.Logger.Warn("[completion_runtime] unauthorized response body close failed", "error", closeErr)
		}
		if detail := strings.TrimSpace(string(body)); detail != "" {
			config.Logger.Warn("[completion_runtime] upstream rejected the managed account token, switching account",
				"account", a.AccountID, "detail", detail)
		}
	}
