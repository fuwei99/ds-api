...
func TestShouldSegmentExpertPrompt_MergedModelReturnsSegments(t *testing.T) {
	// After the 2.4.0 fast/expert/vision merge there is no expert-only gate:
	// segmentation applies to the merged model.
	stdReq := promptcompat.StandardRequest{
		ResolvedModel: "deepseek-v4.1-flash",
		FinalPrompt:   stringRepeat("X", 200000),
	}
	...
	if len(segs) < 2 {
		t.Fatalf("expected segments for merged model, got %d segments", len(segs))
	}
}
